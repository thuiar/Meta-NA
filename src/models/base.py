...
class BaseModel(ABC):
    ...
    def save_networks(self, task, type_ratio):
        for name in self.model_names:
            if isinstance(name, str):
                
                save_path = os.path.join(self.save_dir, '%s_%s_net_%s.pth' % (task, type_ratio, name))
                net = getattr(self, name)
    ...
    def load_networks(self, filename, task, type_ratio):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                load_filename = '%s_%s_net_%s.pth' % (task, type_ratio, name)
