...
class AlignSubNet(nn.Module):
    def __init__(self, args, mode):
        ...
        self.args =args
        self.args.feature_dims = [args.input_dim_l,args.input_dim_a,args.input_dim_v]
        ...
        if args.dataset == 'sims':
            self.ALIGN_WAY['avg_pool'] = self.__sims_avg_pool
    ...
    def __sims_avg_pool(self, text_x, audio_x, video_x, text_lengths, audio_lengths, video_lengths):
        def align(x,min_len,lengths):
            ...
            for index,item in enumerate(x):
                if pad_len[index] == min_len[index]:
                    pad_len[index] = 0
                    pool_size[index] -= 1 
                pad_x = torch.zeros([int(pad_len[index].cpu().numpy()[0]), item.size(-1)]).to(self.args.device)
                item = item[0:int(lengths[index].cpu().numpy()[0]),:]
                item = torch.cat([item,pad_x], dim=0).view(int(min_len[index].cpu().numpy()[0]), int(pool_size[index].cpu().numpy()[0]), -1).mean(dim=1)
    ...
    def forward(self, text_x, audio_x, video_x,text_lengths, audio_lengths, video_lengths):
        # already aligned
        if text_x.size(1) == audio_x.size(1) == video_x.size(1):
            return text_x, audio_x, video_x
        return self.ALIGN_WAY[self.mode](text_x, audio_x, video_x, text_lengths, audio_lengths, video_lengths)